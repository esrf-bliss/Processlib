...
for filename in glob.glob('../tasks/sip/*.sip') :
    ...
    ##src = file(filename)
##    bufferLines = src.read()
##    sipFile.write(bufferLines)
##    sipFile.write('\n')
##    src.close()
sipFile.close()
...
for line in bfile :
    if 'sources' in line :
        ...
        corefiles = ' '.join(glob.glob('../core/src/*.cpp'))
        taskfiles = ' '.join(glob.glob('../tasks/src/*.cpp'))
        line = '%s = %s %s %s' % (begin,corefiles,taskfiles,end)
    whole_line += line
bfile.close()
...
makefile.extra_libs = ['pthread','gsl','gslcblas']
