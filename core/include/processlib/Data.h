...
struct DLL_EXPORT Data
{
    ...
    Data(const Data &aData) : buffer(NULL) { *this = aData; }
    ~Data()
    {
        if (buffer)
            buffer->unref();
    ...
    inline int size() const
    {
        ...
        for (std::vector<int>::const_iterator i = dimensions.begin(); i != dimensions.end(); ++i)
            returnSize *= *i;
        return returnSize;
    }
    ...
    inline Data mask() const
    {
        ...
        if (depth() != 1)
        {
            ...
            {
            case UINT16:
                _make_mask<unsigned short>(*this, aReturnData);
                break;
            case INT16:
                _make_mask<short>(*this, aReturnData);
                break;
            case UINT32:
                _make_mask<unsigned int>(*this, aReturnData);
                break;
            case INT32:
                _make_mask<int>(*this, aReturnData);
                break;
            case UINT64:
                _make_mask<unsigned long long>(*this, aReturnData);
                break;
            case INT64:
                _make_mask<long long>(*this, aReturnData);
                break;
            default:
...
inline std::ostream &operator<<(std::ostream &os, const Data &aData)
{
    ...
    int dimNb = 0;
    for (std::vector<int>::const_iterator i = aData.dimensions.begin(); i != aData.dimensions.end(); ++i)
