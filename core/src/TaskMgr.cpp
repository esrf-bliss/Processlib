...
class TaskLinkWrap : public TaskMgr::TaskWrap
{
  public:
    ...
    virtual ~TaskLinkWrap() { _endLinkTask(_LinkTask); }
...
class TaskSinkWrap : public TaskMgr::TaskWrap
{
  public:
    ...
    virtual ~TaskSinkWrap() { _endSinkTask(_SinkTask); }
    virtual void process()
    {
...
TaskMgr::Task *TaskMgr::Task::copy() const
{
    ...
    for (std::deque<SinkTaskBase *>::const_iterator i = _sinkTaskQueue.begin(); i != _sinkTaskQueue.end(); ++i)
    {
        (*i)->ref();
...
TaskMgr::Task::~Task()
{
    if (_linkTask)
        _linkTask->unref();
    for (std::deque<SinkTaskBase *>::iterator i = _sinkTaskQueue.begin(); i != _sinkTaskQueue.end(); ++i)
        (*i)->unref();
}

TaskMgr::TaskMgr(int priority)
    : _PendingLinkTask(NULL), _initStageFlag(false), _eventCBK(NULL), _priority(priority), _sub_priority(0), _pool(NULL)
{
}

TaskMgr::TaskMgr(const TaskMgr &aMgr)
    : _currentData(aMgr._currentData), _eventCBK(aMgr._eventCBK), _priority(aMgr._priority), _sub_priority(0),
      _pool(NULL)
{
    for (StageTask::const_iterator i = aMgr._Tasks.begin(); i != aMgr._Tasks.end(); ++i)
        _Tasks.push_back((*i)->copy());
}

TaskMgr::~TaskMgr()
{
    for (StageTask::iterator i = _Tasks.begin(); i != _Tasks.end(); ++i)
        delete *i;
}
...
void TaskMgr::getLastTask(std::pair<int, LinkTask *> &aLastLink, std::pair<int, SinkTaskBase *> &aLastSink)
{
    aLastLink.first = -1, aLastLink.second = NULL;
    aLastSink.first = -1, aLastSink.second = NULL;

    for (int i = int(_Tasks.size()) - 1; i >= 0 && (aLastSink.first < 0 || aLastLink.first < 0); --i)
    {
...
{
    for (StageTask::iterator aStageTask = _Tasks.begin(); aStageTask != _Tasks.end(); ++aStageTask)
    {
