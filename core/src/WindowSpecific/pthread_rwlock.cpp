...
int pthread_rwlock_rdlock(pthread_rwlock_t *l)
{
    ...
    {
        DWORD state = SignalObjectAndWait(l->mutex, l->sema_read, INFINITE, FALSE);
        WaitForSingleObject(l->mutex, INFINITE);
    }
    ++(l->reader_count);
    --(l->nb_waiting_reader);
...
int pthread_rwlock_wrlock(pthread_rwlock_t *l)
{
    ...
    while (l->reader_count != 0)
    {
        DWORD state = SignalObjectAndWait(l->mutex, l->sema_write, INFINITE, FALSE);
        WaitForSingleObject(l->mutex, INFINITE);
    }
...
int pthread_rwlock_tryrdlock(pthread_rwlock_t *l)
{
    ...
    {
        /* Unlocked to locked */
        if (!InterlockedCompareExchangePointer((PVOID *)l, (void *)0x11, NULL))
            return 0;
        return EBUSY;
    }
    ...
    if (InterlockedCompareExchangePointer((PVOID *)l, (void *)((uintptr_t)state + 16), state) == state)
        return 0;
...
int pthread_rwlock_trywrlock(pthread_rwlock_t *l)
{
    ...
    if (!InterlockedCompareExchangePointer((PVOID *)l, (void *)1, NULL))
        return 0;
...
int pthread_rwlock_unlock(pthread_rwlock_t *l)
{
    ...
    if (l->reader_count < 0) // Known to be an exclusive lock
    {
        ...
        if (l->nb_waiting_writer)                     // writter have the priority
            ...
            ReleaseSemaphore(l->sema_read, l->nb_waiting_reader,
                             NULL);                          // Wake up all readers
