...
RoiCounterTask::RoiCounterTask(RoiCounterManager &aMgr)
    : SinkTask<RoiCounterResult>(aMgr), _type(UNDEF), _x(0), _y(0), _width(0), _height(0)
{
}

RoiCounterTask::RoiCounterTask(const RoiCounterTask &aTask)
    : SinkTask<RoiCounterResult>(aTask), _type(aTask._type), _x(aTask._x), _y(aTask._y), _width(aTask._width),
      _height(aTask._height), _arc_roi(aTask._arc_roi), _mask(aTask._mask), _lut(aTask._lut)
{
}
...
void RoiCounterTask::setArcMask(double centerX, double centerY, double rayon1, double rayon2, double angle_start,
                                double angle_end)
{
    if (rayon1 > rayon2)
    {
    ...
    for (int angle = int((a_start + 90) / 90) * 90; angle < a_end; angle += 90)
    {
    ...
    if (_x < 0 || _y < 0)
    {
        ...
        snprintf(buffer, sizeof(buffer),
                 "RoiCounterTask arc calculation give an origin (%d,%d) which is "
                 "below index 0",
                 _x, _y);
...
void RoiCounterTask::getArcMask(double &centerX, double &centerY, double &rayon1, double &rayon2, double &angle_start,
                                double &angle_end)
{
    if (_type != ARC)
...
static void _get_average_std(const Data &aData, int x, int y, int width, int height, RoiCounterResult &aResult)
{
...
template <class INPUT>
static void _get_average_std_with_mask(const Data &aData, const char *aMaskPt, int x, int y, int width, int height,
...
template <class INPUT>
static void _lut_get_average_std(const Data &data, int x, int y, Data &lut, RoiCounterResult &aResult)
{
    ...
    int nbItems = 1;
    for (std::vector<int>::iterator i = lut.dimensions.begin(); i != lut.dimensions.end(); ++i)
        nbItems *= *i;
...
template <class INPUT>
static void _mask_get_average_std(const Data &data, int x, int y, Data &mask, RoiCounterResult &aResult)
{
    ...
    int nbItems = 1;
    for (std::vector<int>::iterator i = mask.dimensions.begin(); i != mask.dimensions.end(); ++i)
        nbItems *= *i;
...
void RoiCounterTask::_check_roi_with_data_size(Data &data)
{
    ...
    case LUT:
        ...
        if (_lut.dimensions[0] + _x > data.dimensions[0])
            throw ProcessException("RoiLutCounter lut width + origin go outside of "
                                   "the data bounding box");
        if (_lut.dimensions.size() > 1 && _lut.dimensions[1] + _y > data.dimensions[1])
            throw ProcessException("RoiLutCounter lut height + origin got outside of "
                                   "the data bounding box");
        break;
    case ARC:
    case MASK:
        ...
        if (_lut.dimensions[0] + _x > data.dimensions[0])
            throw ProcessException("RoiLutCounter mask width + origin go outside of "
                                   "the data bounding box");
        if (_lut.dimensions.size() > 1 && _lut.dimensions[1] + _y > data.dimensions[1])
            throw ProcessException("RoiLutCounter mask height + origin got outside "
...
void RoiCounterTask::process(Data &aData)
{
    ...
    } else if (_mask.dimensions == aData.dimensions)
    {
        ...
        {
        case Data::UINT8:
            _get_average_std_with_mask<unsigned char>(aData, (char *)_mask.data(), _x, _y, _width, _height, aResult);
            break;
        case Data::INT8:
            _get_average_std_with_mask<char>(aData, (char *)_mask.data(), _x, _y, _width, _height, aResult);
            break;

        case Data::UINT16:
            _get_average_std_with_mask<unsigned short>(aData, (char *)_mask.data(), _x, _y, _width, _height, aResult);
            break;
        case Data::INT16:
            _get_average_std_with_mask<short>(aData, (char *)_mask.data(), _x, _y, _width, _height, aResult);
            break;
        case Data::UINT32:
            _get_average_std_with_mask<unsigned int>(aData, (char *)_mask.data(), _x, _y, _width, _height, aResult);
            break;
        case Data::INT32:
            _get_average_std_with_mask<int>(aData, (char *)_mask.data(), _x, _y, _width, _height, aResult);
            break;
        case Data::UINT64:
            _get_average_std_with_mask<unsigned long long>(aData, (char *)_mask.data(), _x, _y, _width, _height,
                                                           aResult);
            break;
        case Data::INT64:
            _get_average_std_with_mask<long long>(aData, (char *)_mask.data(), _x, _y, _width, _height, aResult);
            break;

        case Data::FLOAT:
            _get_average_std_with_mask<float>(aData, (char *)_mask.data(), _x, _y, _width, _height, aResult);
            break;
        case Data::DOUBLE:
            _get_average_std_with_mask<double>(aData, (char *)_mask.data(), _x, _y, _width, _height, aResult);
            break;
        default:
            break; // error
        }
