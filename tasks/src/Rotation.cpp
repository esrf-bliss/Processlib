...
Rotation::Rotation() : LinkTask(), m_type(R_90) {}

Rotation::Rotation(const Rotation &other) : LinkTask(other), m_type(other.m_type) {}
...
void _rotate_270(Data &aSrc, Data &aDst)
{
    ...
    for (int lineId = 0; lineId < aSrc.dimensions[1]; ++lineId)
    {
        ...
        for (int columnId = 0; columnId < aSrc.dimensions[0]; ++columnId, ++aSrcPt, aDstColumnPt -= aSrc.dimensions[1])
            *aDstColumnPt = *aSrcPt;
    }
