...
static INPUT max_value(const INPUT &)
{
 ...
 */
template <class INPUT>
inline void _default_binning(Data &aSrcData, Data &aDstData, int xFactor, int yFactor)
{
...
static void _binning2x2(Data &aSrcData, Data &aDstData, int Factor)
{
    ...
    {
        for (int columnId = 0; columnId < aSrcData.dimensions[0];
...
Binning::Binning(const Binning &anOther) : LinkTask(anOther), mXFactor(anOther.mXFactor), mYFactor(anOther.mYFactor) {}
...
Data Binning::process(Data &aData)
{
    ...
    else if (!aData.empty())
    {
        ...
        {
            if (((mXFactor == 2 && mYFactor == 2) || (mXFactor == 4 && mYFactor == 4) ||
                 (mXFactor == 8 && mYFactor == 8) || (mXFactor == 16 && mYFactor == 16) ||
                ...
                !(aData.dimensions[1] % mYFactor))
            {
                ...
                {
                case Data::UINT8:
                    _binning2x2<unsigned char>(aData, aNewData, mXFactor);
                    break;
                case Data::UINT16:
                    _binning2x2<unsigned short>(aData, aNewData, mXFactor);
                    break;
                case Data::UINT32:
                    _binning2x2<unsigned int>(aData, aNewData, mXFactor);
                    break;
                default:
            ...
            } else // DEFAULT case is not optimized
            {
                ...
                {
                case Data::UINT8:
                    _default_binning<unsigned char>(aData, aNewData, mXFactor, mYFactor);
                    break;
                case Data::UINT16:
                    _default_binning<unsigned short>(aData, aNewData, mXFactor, mYFactor);
                    break;
                case Data::UINT32:
                    _default_binning<unsigned int>(aData, aNewData, mXFactor, mYFactor);
                    break;
                default:
                    throw ProcessException("Binning : Data type not managed");
                    break;
