...
static inline INPUT max(INPUT a, INPUT b)
{
    return a > b ? a : b;
}
template <class INPUT>
static inline INPUT min(INPUT a, INPUT b)
{
    return a < b ? a : b;
}
#endif
...
void BpmTask::_treat_image(const Data &aSrc, Buffer &projection_x, Buffer &projection_y, BpmResult &aResult)
{
 ...
 */
template <class INPUT, class SUMMTYPE>
void BpmTask::_tune_projection(const Data &aSrc, Buffer &projection_x, Buffer &projection_y, const BpmResult &aResult)
{
    SUMMTYPE *aProjectionX = (SUMMTYPE *)projection_x.data;
 ...
 */
template <class INPUT, class SUMMTYPE>
static void _max_intensity(const Data &aSrc, const Buffer &projection_x, const Buffer &projection_y, BpmResult &aResult)
{
    Buffer *aBufferPt = aSrc.buffer;
    INPUT *aSrcPt     = (INPUT *)aBufferPt->data;
...
template <class SUMMTYPE>
double BpmTask::_calculate_fwhm(const Buffer &projectionBuffer, int size, int peak_index, double background_level,
                                int &min_index, int &max_index)
{
    ...
    {
        usedBackgroundLevel =
            double(aProjectionPt[mBorderExclusion] + aProjectionPt[mBorderExclusion + 1] +
                   aProjectionPt[mBorderExclusion + 2] + aProjectionPt[size - mBorderExclusion - 1] +
                   aProjectionPt[size - mBorderExclusion - 2] + aProjectionPt[size - mBorderExclusion - 3]) /
            6.;
    }
    ...
    {
        std::cout << "val_h" << val_h << "val_l" << val_l << "hm" << hm << std::endl;
        return -1.;
    }
 ...
 */
template <class SUMMTYPE>
void BpmTask::_calculate_background(const Buffer &projection, double &background_level, int min_index, int max_index)
{
#if DEBUG
 ...
 */
static inline void _extend_y(double arr[], int size, double ret_arr[], int ret_size)
{
    double offset = arr[0];
...
static inline double _compute_center(double y[], int ly)
{
    ...
    int n;
    for (n = 1; n < ly; n <<= 1)
        ; // Find the next above power of two
    n <<= 1;
    int n1 = n << 2;
#ifdef DEBUG
    printf("n : %d , n1 %d\n", n, n1);
    ...
    if (!error_flag)
    {
        ...
        if (!error_flag)
        {
            ...
            else //'final' distribution
            {
                ...
                double X[3];
                error_flag = _solve(yn1_re + first_point, nb_points, first_point, X);
#ifdef DEBUG
                printf("solve done..\n");
#endif
                if (!error_flag)
...
BpmTask::BpmTask(BpmManager &aMgr)
    : SinkTask<BpmResult>(aMgr), mFwhmTunning(false), mFwhmTunningExtension(1.5), mAoiExtension(4.),
      mBorderExclusion(10),
      //   mGaussFittMax(false),
      //   mAverage(1),
      mThreshold(0), mEnableX(true), mEnableY(true), mEnableBackgroundSubstration(false), mRoiAutomatic(true),
      _RoiX1(-1), _RoiX2(-1), _RoiY1(-1), _RoiY2(-1)
{
}

BpmTask::BpmTask(const BpmTask &aTask)
    : SinkTask<BpmResult>(aTask), mFwhmTunning(aTask.mFwhmTunning), mFwhmTunningExtension(aTask.mFwhmTunningExtension),
      mAoiExtension(aTask.mAoiExtension), mBorderExclusion(aTask.mBorderExclusion),
      //   mGaussFittMax(aTask.mGaussFittMax),
      //   mAverage(aTask.mAverage),
      mThreshold(aTask.mThreshold), mEnableX(aTask.mEnableX), mEnableY(aTask.mEnableY),
      mEnableBackgroundSubstration(aTask.mEnableBackgroundSubstration), mRoiAutomatic(aTask.mRoiAutomatic),
      _RoiX1(aTask._RoiX1), _RoiX2(aTask._RoiX2), _RoiY1(aTask._RoiY1), _RoiY2(aTask._RoiY2)
{
}
...
#define TUNE_FWHM(XorY, WidthorHeight, SUMMTYPE)                                                                       \
    {                                                                                                                  \
        int min_index, max_index;                                                                                      \
        aResult.beam_fwhm_##XorY = _calculate_fwhm<SUMMTYPE>(                                                          \
...
void BpmTask::process(Data &aInputSrc)
{
    ...
    else if (!(_RoiX1 < 0 && _RoiX2 < 0 && _RoiY1 < 0 && _RoiY2 < 0))
    {
    ...
    {
    case Data::UINT8:
        PROCESS(unsigned char, int);
        break;
    case Data::INT8:
        PROCESS(char, int);
        break;
    case Data::UINT16:
        PROCESS(unsigned short, int);
        break;
    case Data::INT16:
        PROCESS(short, int);
        break;
    case Data::INT32:
        ...
        break;
    case Data::UINT32:
        PROCESS(unsigned int, long long);
        break;
    default:
    {
        char aBuffer[256];
    ...
    projection_x->unref();
    projection_y->unref();
    if (!(_RoiX1 < 0 && _RoiX2 < 0 && _RoiY1 < 0 && _RoiY2 < 0))
    {
 ...
 */
void BpmTask::setRoi(int x1, int x2, int y1, int y2)
{
    _RoiX1 = x1, _RoiX2 = x2;
    _RoiY1 = y1, _RoiY2 = y2;
}
//@brief get roi corner
void BpmTask::getRoi(int &x1, int &x2, int &y1, int &y2) const
{
    x1 = _RoiX1, x2 = _RoiX2;
    y1 = _RoiY1, y2 = _RoiY2;
}
