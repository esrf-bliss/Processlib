...
PeakFinderTask::PeakFinderTask(PeakFinderManager &aMgr)
    : SinkTask<PeakFinderResult>(aMgr), _nb_peaks(1), _computing_mode(PeakFinderTask::MAXIMUM)
{
}

PeakFinderTask::PeakFinderTask(const PeakFinderTask &aTask)
    : SinkTask<PeakFinderResult>(aTask), _mask(aTask._mask), _nb_peaks(aTask._nb_peaks),
      _computing_mode(PeakFinderTask::MAXIMUM)
{
}

template <class INPUT>
static void _compute_peaks(const Data &aData, PeakFinderResult &aResult, int computing_mode)
{
