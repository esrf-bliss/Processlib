...
Data _norm(Data &src)
{
    double sum  = 0.;
    int nbItems = 1;
    for (std::vector<int>::const_iterator i = src.dimensions.begin(); i != src.dimensions.end(); ++i)
        nbItems *= *i;
...
static Data _normalize(Data &src)
{
    ...
    {
    case Data::UINT8:
        aReturnData = _norm<unsigned char>(src);
        break;
    case Data::INT8:
        aReturnData = _norm<char>(src);
        break;
    case Data::UINT16:
        aReturnData = _norm<unsigned short>(src);
        break;
    case Data::INT16:
        aReturnData = _norm<short>(src);
        break;
    case Data::UINT32:
        aReturnData = _norm<unsigned int>(src);
        break;
    case Data::INT32:
        aReturnData = _norm<int>(src);
        break;
    case Data::UINT64:
        aReturnData = _norm<unsigned long long>(src);
        break;
    case Data::INT64:
        aReturnData = _norm<long long>(src);
        break;
    case Data::FLOAT:
        aReturnData = _norm<float>(src);
        break;
    default:
...
FlatfieldCorrection::FlatfieldCorrection() : LinkTask() {}

FlatfieldCorrection::FlatfieldCorrection(const FlatfieldCorrection &aFlatField)
    : LinkTask(aFlatField), _flatFieldImage(aFlatField._flatFieldImage)
{
}
...
Data FlatfieldCorrection::process(Data &aData)
{
    if (aData.dimensions == _flatFieldImage.dimensions)
    {
        ...
        {
        case Data::UINT8:
            _div<unsigned char>(aData.data(), aData.size(), _flatFieldImage.data());
            break;
        case Data::UINT16:
            _div<unsigned short>(aData.data(), aData.size(), _flatFieldImage.data());
            break;
        case Data::UINT32:
            _div<unsigned int>(aData.data(), aData.size(), _flatFieldImage.data());
            break;
        case Data::INT32:
            _div<int>(aData.data(), aData.size(), _flatFieldImage.data());
            break;
        default:
