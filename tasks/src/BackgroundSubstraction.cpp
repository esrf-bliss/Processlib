...
static void _substract_in_place_offset(void *src, int aSize, void *background, int offset)
{
    ...
    for (int i = aSize / sizeof(INPUT); i; --i, ++aSrcPt, ++aBackPt)
    {
        INPUT srcValue = *aSrcPt >= maxValue ? std::numeric_limits<INPUT>::max() : *aSrcPt + offset;
        if (srcValue > *aBackPt)
...
template <class INPUT>
static void _substract_offset(void *src, void *dst, int aSize, void *background, int offset)
{
    INPUT *aSrcPt, *aBackPt, *aDestPt;
    aSrcPt         = (INPUT *)src;
    aBackPt        = (INPUT *)background;
...
BackgroundSubstraction::BackgroundSubstraction() : LinkTask(), _offset(0) {}
...
{
    if (aData.type == _backgroundImageData.type && aData.dimensions == _backgroundImageData.dimensions)
    {
        ...
        if (_processingInPlaceFlag)
        {
            if (_offset > 0)
            {
                ...
                {
                case Data::INT8:
                    _substract_in_place_offset<char>(aData.data(), aData.size(), _backgroundImageData.data(), _offset);
                    break;
                case Data::UINT8:
                    _substract_in_place_offset<unsigned char>(aData.data(), aData.size(), _backgroundImageData.data(),
                                                              _offset);
                    break;
                case Data::INT16:
                    _substract_in_place_offset<short>(aData.data(), aData.size(), _backgroundImageData.data(), _offset);
                    break;
                case Data::UINT16:
                    _substract_in_place_offset<unsigned short>(aData.data(), aData.size(), _backgroundImageData.data(),
                                                               _offset);
                    break;
                case Data::INT32:
                    _substract_in_place_offset<int>(aData.data(), aData.size(), _backgroundImageData.data(), _offset);
                    break;
                case Data::UINT32:
                    _substract_in_place_offset<unsigned int>(aData.data(), aData.size(), _backgroundImageData.data(),
                                                             _offset);
                    break;
                default:
                    break;
            ...
            } else
            {
                ...
                {
                case Data::INT8:
                    _substract_in_place<char>(aData.data(), aData.size(), _backgroundImageData.data());
                    break;
                case Data::UINT8:
                    _substract_in_place<unsigned char>(aData.data(), aData.size(), _backgroundImageData.data());
                    break;
                case Data::INT16:
                    _substract_in_place<short>(aData.data(), aData.size(), _backgroundImageData.data());
                    break;
                case Data::UINT16:
                    _substract_in_place<unsigned short>(aData.data(), aData.size(), _backgroundImageData.data());
                    break;
                case Data::INT32:
                    _substract_in_place<int>(aData.data(), aData.size(), _backgroundImageData.data());
                    break;
                case Data::UINT32:
                    _substract_in_place<unsigned int>(aData.data(), aData.size(), _backgroundImageData.data());
                    break;
                default:
                    break;
                }
        ...
        } else
        {
            ...
            if (_offset > 0)
            {
                ...
                {
                case Data::INT8:
                    _substract_offset<char>(aData.data(), aNewData.data(), aData.size(), _backgroundImageData.data(),
                                            _offset);
                    break;
                case Data::UINT8:
                    _substract_offset<unsigned char>(aData.data(), aNewData.data(), aData.size(),
                                                     _backgroundImageData.data(), _offset);
                    break;
                case Data::INT16:
                    _substract_offset<short>(aData.data(), aNewData.data(), aData.size(), _backgroundImageData.data(),
                                             _offset);
                    break;
                case Data::UINT16:
                    ...
                    break;
                case Data::INT32:
                    _substract_offset<int>(aData.data(), aNewData.data(), aData.size(), _backgroundImageData.data(),
                                           _offset);
                    break;
                case Data::UINT32:
            ...
            } else
            {
                ...
                {
                case Data::INT8:
                    _substract<char>(aData.data(), aNewData.data(), aData.size(), _backgroundImageData.data());
                    break;
                case Data::UINT8:
                    _substract<unsigned char>(aData.data(), aNewData.data(), aData.size(), _backgroundImageData.data());
                    break;
                case Data::INT16:
                    _substract<short>(aData.data(), aNewData.data(), aData.size(), _backgroundImageData.data());
                    break;
                case Data::UINT16:
                    ...
                    break;
                case Data::INT32:
                    _substract<int>(aData.data(), aNewData.data(), aData.size(), _backgroundImageData.data());
                    break;
                case Data::UINT32:
                    _substract<unsigned int>(aData.data(), aNewData.data(), aData.size(), _backgroundImageData.data());
                    break;
                default:
                    break;
                }
