...
class PROCESSLIB_EXPORT BpmResult
{
  public:
    BpmResult()
        : beam_intensity(0), beam_center_x(0.), beam_center_y(0.), beam_fwhm_x(0.), beam_fwhm_y(0.), max_pixel_value(0),
          max_pixel_x(0), max_pixel_y(0), frameNumber(-1), AOI_automatic(true), AOI_extension_factor(4), AOI_min_x(-1),
          AOI_max_x(-1), AOI_min_y(-1), AOI_max_y(-1), border_exclusion(10), mBackgroundLevelx(0.),
          mBackgroundLevely(0.), mBackgroundLevelTunex(0.), mBackgroundLevelTuney(0.), errorCode(BpmManager::OK),
          timestamp(-1.)
    {
    }
...
class PROCESSLIB_EXPORT BpmTask : public SinkTask<BpmResult>
{
  public:
    ...
    void getRoi(int &x1, int &x2, int &y1, int &y2) const;
  ...
  private:
    ...
    void _treat_image(const Data &aSrc, Buffer &projection_x, Buffer &projection_y, BpmResult &aResult);
    template <class IN, class SUMMTYPE>
    void _tune_projection(const Data &aSrc, Buffer &, Buffer &, const BpmResult &);
    template <class SUMMTYPE>
    double _calculate_fwhm(const Buffer &projection, int size, int max_index, double background_level, int &, int &);
    template <class SUMMTYPE>
    void _calculate_background(const Buffer &projection, double &background_level, int min_index, int max_index);
};
} // namespace Tasks
#else
namespace Tasks {
